...
func TestLogTimer_Duration(t *testing.T) {
	...
	d := tmr.Duration()
	testutil.AssertEqual(t, CallbackDuration, d, "Durations did not match.")
	tmr.Stop()
}
...
func TestNewTimer(t *testing.T) {
	...
	tmr.Stop()
	testutil.AssertTrue(t, ts1.received, "TestNewTimer: No message received.")
	testutil.AssertEqual(t, ts1.msgCnt, MaxIterations, "TestNewTimer: did not receive all messages")
}
...
func TestNewLocalTimer(t *testing.T) {
	...
	loopStop.Stop()
	tmr.Stop()
	testutil.AssertTrue(t, ts1.received, "TestNewLocalTimer: No message received.")
	testutil.AssertEqual(t, ts1.msgCnt, MaxIterations,
...
func TestLogTimer_Stop(t *testing.T) {
	...
	testutil.AssertFalse(t, ts1.received, "TestLogTimer_Stop: No message received.")
}
