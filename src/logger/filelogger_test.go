...
func TestFile(t *testing.T) {
	...
	t.Run("NewStatic=2", func(t *testing.T) {
		...
		testutil.AssertNil(t, err, fmt.Sprintf("%s; File:\"%s\"\n", err, testName))
		testutil.AssertNotNil(t, l, fmt.Sprintf("*LogFile is nil: \"%s\"\n", testName[1]))
...
func TestLimitedFile(t *testing.T) {
	...
	t.Run(runName, func(t *testing.T) {
		...
		p := l.LogPolicy()
		testutil.AssertTrue(t, p.IsSizeLimited(), "Expected size limited file policy, got "+p.String())
		testutil.AssertFalse(t, p.isNone(), "Expected size limited file policy, got "+p.String())
		testutil.AssertFalse(t, p.IsDaily(), "Expected size limited file policy, got "+p.String())
		testutil.AssertFalse(t, p.IsTimed(), "Expected size limited file policy, got "+p.String())
		...
		testutil.AssertNotEmptyString(t, names[0], fmt.Sprintf("Value: \"%s\"\n", testName))
		...
		testutil.AssertStringsNotEqual(t, names[0], names[1], "Expected two different files. "+
			names[0]+" "+names[1])
	})
}
...
func TestTimedFile(t *testing.T) {
	...
	for i := 0; i < numRuns; i++ {
		t.Run(testName, func(t *testing.T) {
			...
			testutil.AssertNil(t, err, fmt.Sprintf("File \"%s\"\n", testName))
			testutil.AssertNotNil(t, l, fmt.Sprintf("*LogFile is nil: \"%s\"\n", testName))

			p := l.LogPolicy()
			testutil.AssertFalse(t, p.IsSizeLimited(), "Expected timed file policy, got "+p.String())
			testutil.AssertFalse(t, p.isNone(), "Expected timed file policy, got "+p.String())
			testutil.AssertFalse(t, p.IsDaily(), "Expected timed file policy, got "+p.String())
			testutil.AssertTrue(t, p.IsTimed(), "Expected timed file policy, got "+p.String())

			name1 := l.LogFilename()
			testutil.AssertNotEmptyString(t, name1, "Filename: "+name1)
			...
			testutil.AssertNotEmptyString(t, name2, "Filename: "+name2)

			testutil.AssertStringsNotEqual(t, name1, name2,
				fmt.Sprintf("File 1: \"%s\" File 2 \"%s\".\n", name1, name2))

			_, ok1 := os.Stat(name1)
			_, ok2 := os.Stat(name2)
			testutil.AssertNil(t, ok1, fmt.Sprintf("%s; File: \"%s\".", ok1, name1))
			testutil.AssertNil(t, ok2, fmt.Sprintf("%s; File: \"%s\".", ok2, name2))
...
func TestDailyFile(t *testing.T) {
	...
	testutil.AssertNil(t, err, fmt.Sprintf("Error opening \"%s\"\n", testName))
	testutil.AssertNotNil(t, l, fmt.Sprintf("*LogFile is nil: \"%s\"\n", testName))

	p := l.LogPolicy()
	testutil.AssertFalse(t, p.IsSizeLimited(), "Expected Daily file policy, got "+p.String())
	testutil.AssertFalse(t, p.isNone(), "Expected Daily file policy, got "+p.String())
	testutil.AssertTrue(t, p.IsDaily(), "Expected Daily file policy, got "+p.String())
	testutil.AssertFalse(t, p.IsTimed(), "Expected Daily file policy, got "+p.String())
